...
treatment = c("IgG","aCD4","IgG")
condition = c("Control","TBI","TBI")
region = c("Thalamus","Thalamus","Thalamus")
...
sample_description = paste0(condition,"-",treatment)
meta = as.data.frame(cbind(treatment,condition,sample_description))
rownames(meta) = ids
meta
...
treatment = sapply(samplename,function(x)meta[x,"treatment"])
...
sobject$treatment = treatment
sobject$condition = condition

#check
table(sobject$sample_description,sobject$orig.ident)
table(sobject$treatment,sobject$condition)
table(sobject$treatment,sobject$sample_description)
```
...
if (organism %in% c("Human","Mouse","Rat")){
  if (organism == "Human") {
    sexgenesfemale = c("XIST","TSIX")
    sexgenesmale = c("EIF2S3Y","DDX3Y")
  } else {
    sexgenesfemale = c("Xist","Tsix")
    sexgenesmale = c("Eif2s3y","Ddx3y")
  }
  female = GetAssayData(sobject,slot = "counts")[sexgenesfemale,]
  female = colSums(female)
  male = GetAssayData(sobject,slot = "counts")[sexgenesmale,]
  male = colSums(male)
  sex = colnames(sobject)
  sex[male>0] = "Male"
  sex[female == 0] = "Male"
  sex[sex != "Male"] = "Female"
  ...
  print(VlnPlot(sobject,c(sexgenesmale,sexgenesfemale),group.by = "sex"))
  table(sobject$sex,sobject$sample_description)
  sobject$fulldescription = paste0(sobject$sample_description,"_",sobject$sex)
  VlnPlot(sobject,"nFeature_RNA",group.by = "fulldescription")
}
...
counts = as.data.frame(sobject@assays$RNA@counts)
gzipfile = gzfile(file.path(dir,"Spreadsheets","counts.gz"),"w")
write.csv(counts,gzipfile)
close(gzipfile)

cellassignment = sobject$orig.ident
write.csv(cellassignment,file.path(dir,"Spreadsheets","unfiltered_sampleID.csv"),quote = F)
```
packageVersion("Seurat")
packageVersion("ggplot2")
packageVersion("dplyr")
