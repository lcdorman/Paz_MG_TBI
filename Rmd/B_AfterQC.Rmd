...
VlnPlot(sobject,c("Mbp","Hexb","Aif1","Slc1a3","percent.mito"),group.by = "seurat_clusters",pt.size = 0,ncol = 2,sort = T)
```
****
...
for (gene in c("Mbp","Hexb","Aif1","Slc1a3")){
  setEPS()
  postscript(file.path(dir,"QC",paste0(gene,"_initial_clustering_vln.eps")))
  print(VlnPlot(sobject,gene,group.by = "seurat_clusters",pt.size = 0,ncol = 2) + stat_summary(fun = "median",geom = "point",shape = 95, size = 5, col = "black") + NoLegend())
dev.off()
}
...
setEPS()
postscript(file.path(dir,"QC","celltype_labels_dimplot.eps"))
DimPlot(sobject,group.by = "celltype",label = T)+NoLegend()
dev.off()

setEPS()
postscript(file.path(dir,"QC","cluster_labels_dimplot.eps"))
DimPlot(sobject,group.by = "seurat_clusters",label = T)+NoLegend()
dev.off()

setEPS()
postscript(file.path(dir,"QC","celltype_genes_featureplot.eps"))
FeaturePlot(sobject,c("Mbp","Hexb","Aif1","Slc1a3"))
dev.off()
```

Selected out any cells that have higher than "2" expression of mbp
```{r}
...
Regress % mito, nFeature RNA, remove sex genes
Keep microglia only (remove 15 [astro] and 7 [oligo])
Remove all cells with mbp > 2 (assuming some myelin contamination, wanted to clean up)
Recluster, same regression, remove sex genes
resolution 0.5
Combined H1 and H2 groups of clusters: a) very few upregulated genes (<5 for 5,6,7,9) b) more closely related than the DAM clusters were to each other 
